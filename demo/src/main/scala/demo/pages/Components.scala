...
object Components {
  ...
  class Backend(scope: BackendScope[Unit, State]) {
    def handleNavItemSelect(args: Seq[UndefOr[String]]) = {
      Console.println(s"handleNavItemSelect: $args")
      if (args.length > 1) {
        if (args(1).isDefined) {
          val href = args(1).get
        ...
        <.div(^.className := "row",
          ...
          <.div(^.className := "col-md-3",
            Affix(Affix.Props(addClasses = "bs-docs-sidebar hidden-print",
              ...
              Nav.withRef("sideRef")(Nav.Nav(addClasses = "bs-docs-sidenav", activeHref = "", onSelect = (args: Seq[UndefOr[String]]) => B.handleNavItemSelect(args)),
