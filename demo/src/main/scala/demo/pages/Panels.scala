...
object Panels {
  ...
  val headerSource =
    """
      ...
      |      , Panel(Panel.Props(header = <.h3("Panel title"): ReactNode), "Panel content")
      |)
    """.stripMargin
  ...
  val footerSource =
    """
      |<.div(
      |      Panel(Panel.Props(footer = "Panel heading without title": ReactNode), "Panel content")
      |      , Panel(Panel.Props(footer = <.h3("Panel title"): ReactNode), "Panel content")
      |)
    """.stripMargin
  ...
  val content = Section("panels", <.span("Panels ", <.small("Panel, PanelGroup, Accordion"))
    ...
    , SubSection("panels-footer", "Panel with footer",
      ...
      footerContent())
    , SubSection("panels-contextual", "Panel with footer",
